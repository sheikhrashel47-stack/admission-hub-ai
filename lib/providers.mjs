...
export const FREEMODELS = [
  ...
  { pid: 'cerebras', id: 'c3', label: 'Cerebras · Llama 3.3-70B', model: 'llama-3.3-70b', speed: 5, quality: 3, coding: 4, context: 131072 },
...
export function pickChain({ model, mode, task }) {
  let list;
  if (model && model !== 'auto') {
...
export async function* streamAnswer(messages, { model, mode, onAttempt, signal } = {}) {
  const task = classifyTask(messages[messages.length - 1]?.content || '', mode);
  const chain = pickChain({ model, mode, task });
  const errors = [];
